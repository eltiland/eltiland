...
public class NodeManagerEditorPanel extends BrixGenericPanel<BrixNode> {
    public NodeManagerEditorPanel(String id, IModel<BrixNode> model) {
        ...
        final String root = SitePlugin.get().getSiteRootPath();
        ...
        AbstractReadOnlyModel<String> pathModel = new AbstractReadOnlyModel<String>() {
            ...
            public String getObject() {
                ...
                String CMS_ROOT = "/brix:root/brix:web/brix:site/CMS/";
                String nodePath = getNode().getPath();
                if(!nodePath.startsWith(CMS_ROOT)) {
                    return "Служебная страница. Ссылки не поддерживаются!";
                }

                return "http://eltiland.ru/c/"+ nodePath.substring(CMS_ROOT.length());
            }
        };
        add(new WebMarkupContainer("permalink").add(new AttributeModifier("value", pathModel)));
