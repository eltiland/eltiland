package com.eltiland.ui.library.view.kind.gallery;
...
/**
 * Record panel for gallery view.
 *
 * @author Aleksey Plotnikov.
 */
public class RecordGalleryPanel extends BaseEltilandPanel<LibraryRecord> {

    protected static final Logger LOGGER = LoggerFactory.getLogger(RecordGalleryPanel.class);
     ...
     *
     * @param id           markup id.
     * @param recordIModel library record model.
     */
    public RecordGalleryPanel(String id, IModel<LibraryRecord> recordIModel) {
        super(id, recordIModel);
        ...
        add(new Label("name", StringUtils.truncate(getModelObject().getName(), 70)));
        add(new Label("description", StringUtils.truncate(getModelObject().getDescription(), 70)));
        ...
        add(new AjaxEventBehavior("onclick") {
            ...
            protected void onEvent(AjaxRequestTarget target) {
                ...
                if (clazz.equals(LibraryDocumentRecord.class) || clazz.equals(LibraryPresentationRecord.class)) {
                    canBeViewed = RecordGalleryPanel.this.getModelObject().isPublished();
                } else {
                ...
                if (canBeViewed) {
                    throw new RestartResponseException(RecordViewPage.class,
                            new PageParameters().add(RecordViewPage.PARAM_ID,
                                    RecordGalleryPanel.this.getModelObject().getId()));
                } else {
