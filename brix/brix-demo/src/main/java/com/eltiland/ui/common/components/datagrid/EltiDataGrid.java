 ...
 * @author Matej Knopp
 */
public class EltiDataGrid<D extends IDataSource<T>, T> extends AbstractGrid<D, T> implements IPageable {

    private static final long serialVersionUID = 1L;

    /**
     * Crates a new {@link com.eltiland.ui.common.components.datagrid.EltiDataGrid} instance.
     *
     * @param id      component id
     * @param model   model to access the {@link com.inmethod.grid.IDataSource} instance used to fetch the data
     * @param columns list of grid columns
     */
    public EltiDataGrid(String id, IModel<D> model, List<IGridColumn<D, T>> columns) {
     ...
     * @param columns    list of grid columns
     */
    public EltiDataGrid(String id, D dataSource, List<IGridColumn<D, T>> columns) {
        this(id, Model.of(dataSource), columns);
    }

    private class Body extends EltiDataGridBody<D, T> {
        ...
        protected Collection<IGridColumn<D, T>> getActiveColumns() {
            return EltiDataGrid.this.getActiveColumns();
        }

        @Override
        protected D getDataSource() {
            return EltiDataGrid.this.getDataSource();
        }

        @Override
        protected int getRowsPerPage() {
            return EltiDataGrid.this.getRowsPerPage();
        }

        @Override
        protected IGridSortState getSortState() {
            return EltiDataGrid.this.getSortState();
        }

        @Override
        protected boolean isItemSelected(IModel<T> itemModel) {
            return EltiDataGrid.this.isItemSelected(itemModel);
        }
    ...
    /**
     * Returns the {@link com.inmethod.grid.IDataSource} instance this data grid uses to fetch the data.
     *
     * @return {@link com.inmethod.grid.IDataSource} instance
     */
     ...
     *
     * @param rowsPerPage how many rows (max) should be displayed on one page
     * @return <code>this</code> (useful for method chaining)
     */
    public EltiDataGrid<D, T> setRowsPerPage(int rowsPerPage) {
     ...
     * {@link com.inmethod.grid.common.AbstractPageableView#UNKOWN_COUNT} in case the count can't be determined.
     *
     * @return total count of items or {@value com.inmethod.grid.common.AbstractPageableView#UNKOWN_COUNT}
     */
     ...
     * the same time.
     *
     * @param cleanSelectionOnPageChange whether the current page change should deselect all selected items
     * @return <code>this</code> (useful for method chaining)
     */
    public EltiDataGrid<D, T> setCleanSelectionOnPageChange(boolean cleanSelectionOnPageChange) {
     ...
     * when {@link com.inmethod.grid.common.AbstractGrid#update()} method is called.
     *
     * @param itemModel model used to access the item
     */
    @Override
    ...
    public WebMarkupContainer findParentRow(Component child) {
        return child.findParent(EltiDataGridBody.Data.RowItem.class);
    }
}
